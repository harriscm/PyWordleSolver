...
def _runCycle(word, wordlist):
    ...
    while greenWord.lower() != word.lower():
        greenWord = click.prompt(f"Type {word}, but only capitalize the GREEN letters")

    yellowWord = ""
    while yellowWord.lower() != word.lower():
        yellowWord = click.prompt(f"Type {word}, but only capitlize the YELLOW letters")
...
def solve():
    ...
    while iterator <= 6 and len(remainingWords) != 0:
        remainingWords = _runCycle(remainingWords[0], remainingWords)
        iterator = iterator + 1
...
WORDLIST = [
    "audio",  # This is the first word tried each time, it's a good one
    "aback",
    "abase",
    "abate",
    "abbey",
